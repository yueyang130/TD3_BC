...
class SingleCritic(nn.Module):
    ...
    def forward(self, state, action):
        ...
        return final_q1, penultimate_q1, 
...
class DDPG_BC(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.critic = SingleCritic(state_dim, action_dim, qf_layer_norm, last_act_bound=last_act_bound, dropout_prob=dropout_prob).to(device)
    ...
    def train(self, replay_buffer, two_sampler=False):
        ...
        target_Q = reward + not_done * self.discount * target_Q1.detach()

        # Get current Q estimates
        current_Q1, phi_q1 = self.critic(state, action)
        ...
        if self.dr3_coef > 0:
            _, phi_q1_prime = self.critic(next_state, next_action)
            dr3_loss = torch.sum(phi_q1_prime * phi_q1, dim=1).mean()
            critic_loss += dr3_loss * self.dr3_coef
        else:
            dr3_loss = torch.tensor([0.0])
